...
import { approveCoachAction, rejectCoachAction } from "./actions";

type DashboardCoachesPageProps = {
...
export default async function DashboardCoachesPage({
  searchParams,
}: DashboardCoachesPageProps) {
  const [viewer, params] = await Promise.all([
    requireAccountViewer("/dashboard/coaches", ["chapter_admin"]),
    searchParams,
  ]);
  const coaches = await listPendingCoaches({
